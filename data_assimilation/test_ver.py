...
# Q0. カオスとは
# 小さな初期値の差が時間経過で膨大に大きくなるような環境や場所のこと

# Q1. L96 モデルの方程式を言葉で説明してください
# ANS: dxdy = (next1 - prev2) * prev1 - current + F 
# で風速などが周りに影響を与え合ったり、現在の状態は-currentとすることで収束しようとしている状態
# +Fとすることで変化量がおおきく、収束しない


# Q2. F=8 のとき何が起きるか？
# ANS:エネルギーの総和が安定しない、一定値にならないなど


# Q3. RK4 はオイラー法と比べて何が違うか？
# ANS: オイラー法では1点の傾きしか見ないが、Rk4では多点の傾きを見ることで更に制度が高く予測がｄ消失r


# Q4. RK4 の更新式を書いてください（k1~k4 を使って）
# ANS:
#   k1 = lorenz(x +  dt, F)
#   k2 =lorenz(x +  k2*0.5*dt, F)
#   k3 = lorenz(x + k3*0.5*dt, F)
#   k4 = lorenz(x + k4*dt, F)
#   x_new = x + (dt/6)*(k1 + k*2 + k*3+k4)
...
# ANS: 1/n *(true - error) ** 2 です

# Q9. 0.2 MTU = 1日 とする妥当性をどのように確認するか？
# ANS: 誤差倍増時間が現実世界で2日なので, に倍になる時間のMTUを2日遠く
...
# ANS:
#   K  = Pb @ H.T np.linear( H @  Pb @ H.T  + R)
#   xa = xb + K ( y_obs - xb @ H.T)
#   Pa = (I - K @ H) @ Pa
